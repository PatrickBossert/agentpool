...
Browser  →  React SPA (ui/)          →  FastAPI (api/)   →  SQLite per project
                                     ...
                                     →  /orchestrate      →  PAM crew → child crews
...
Starts ChromaDB on `:8002` — the only Docker service. Skip it entirely if `CHROMA_API_KEY` is set, in which case Chroma Cloud is used instead.
...
Click **Run Pipeline** on the Dashboard. This calls the PAM (Project Automation Manager) orchestration endpoint at `POST /orchestrate` directly.
...
Caddy serves the React build at `/dashboard` and proxies `/api` to FastAPI.
...
**A review gate never gets answered**
Nothing notifies a reviewer that a gate is open — n8n relayed those to Slack and was retired in SP50 with no replacement. Open the project's **Reviews** page to find waiting gates. An unanswered gate times out after 24 hours.
