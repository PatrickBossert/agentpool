...
function GanttChart({
  ...
  // Interview window in working days (recalculates during drag)
  ...
  // Holidays visible in this date range
    ...
    <div className="mb-6 select-none">
      ...
      >
        ...
        {nonWorkingRanges.map(r => {
          ...
          // +1 so the band covers the full end day (inclusive)
        ...
        {sorted.map((m, idx) => {
          ...
          // Short locale-formatted date for gantt label
...
function DateControls({
  ...
  return (
    <div className="flex items-end gap-6 flex-wrap bg-gray-50 border border-gray-100 rounded-2xl px-5 py-4 mb-6">
      <div>
...
function ScheduleSummary({ milestones }: { milestones: Milestone[] }) {
  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
      ...
      {next && (
        <div className="col-span-2 sm:col-span-4 rounded-xl border border-gray-100 bg-gray-50 p-3 flex items-center gap-2">
...
export default function Schedule() {
  ...
  // End date derived from schedule controls
  const endDate = addDays(schedStart, durationWeeks * 7)

  // Public holidays for this locale and date window
  const holidays: PublicHoliday[] = locale
    ? getPublicHolidays(locale, schedStart, endDate)
    : []

  // Set of excluded dates (holidays + NWR) for working-day calculations
  const excludedDates = buildExcludedDateSet(holidays, nonWorkingRanges)

  // Persist schedule window to project settings whenever it changes
  const saveSchedule = useCallback((start: string, weeks: number) => {
    ...
    projectsApi.updateSettings(slug, { ...settings, sched_start: start, sched_duration_weeks: weeks })
      .catch(() => { /* non-critical — UI already shows correct values */ })
  }, [slug, settings])

  // Initialise schedule window: prefer stored values, fall back to milestone-date inference
  useEffect(() => {
    if (initialized) return
    if (settings === undefined && !settingsError) return  // still loading
  ...
  const assignDates = async () => {
    if (!slug || !milestones.length) return
    setAssignBusy(true)
    try {
  ...
  return (
    <div className="px-6 py-8 max-w-4xl mx-auto">
      <div className="flex items-start justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            Set start date and duration to auto-assign milestone dates, then drag Gantt markers to shift that milestone and all following ones.
          </p>
        </div>
        <div className="flex gap-2 flex-shrink-0">
          <button
            ...
            className="flex items-center gap-1.5 text-xs px-3 py-2 rounded-lg border border-gray-200 text-gray-600 hover:border-gray-400 hover:text-gray-900 transition-colors disabled:opacity-40"
          >
          ...
          <button
            ...
            className="flex items-center gap-1.5 text-xs px-3 py-2 rounded-lg bg-teal-600 text-white hover:bg-teal-700 transition-colors"
          >
            <Plus size={12} />
            Add milestone
          </button>
      ...
      <p className="text-xs text-gray-400 mt-8 text-center">
        Date changes take effect immediately. The PMO reads this schedule when planning escalations and reminders. —{' '}
        <Link to={`/${slug}`} className="text-teal-500 hover:underline">Back to Dashboard</Link>
      </p>
    </div>
  )
}
