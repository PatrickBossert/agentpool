...
const GANTT_STYLE: Record<MilestoneRAG, { border: string; bg: string; text: string }> = {
  complete:    { border: 'border-teal-400',  bg: 'bg-teal-500',   text: 'text-white' },
  overdue:     { border: 'border-red-400',   bg: 'bg-red-500',    text: 'text-white' },
  due_soon:    { border: 'border-amber-400', bg: 'bg-amber-400',  text: 'text-white' },
  on_track:    { border: 'border-green-400', bg: 'bg-slate-800',  text: 'text-green-400' },
  unscheduled: { border: 'border-gray-500',  bg: 'bg-slate-700',  text: 'text-gray-400' },
}
...
export default function GanttReadOnly({
  ...
  return (
    <div className="mb-2 select-none">
      ...
      >
        {/* Inner chart area — inset 40px from each edge so the dark background has side
            margins, and the centered 80px milestone labels at both ends never spill out. */}
        <div className="absolute inset-y-0 left-10 right-10">
          ...
          {lDate && cDate && (
            <>
              ...
              >
                <span className="text-[8px] text-teal-300 font-bold tracking-widest uppercase whitespace-nowrap bg-slate-800/70 px-1.5 rounded-sm">
                  INTERVIEWS
                </span>
              </div>
            </>
      ...
      <div className="flex items-center justify-between mt-1.5 px-1 text-[10px] text-gray-400">
        ...
        <div className="flex items-center gap-5">
          {lDate && cDate && (() => {
            ...
            return (
              <span className="flex items-center gap-1.5">
                <span className="w-3 h-3 rounded bg-teal-400/30 border border-teal-400/60 inline-block" />
          ...
          {(nonWorkingRanges.some(r => r.end_date >= startDate && r.start_date <= endDate) || visibleHolidays.length > 0) && (() => {
            ...
            return (
              <span className="flex items-center gap-1.5">
                <span className="w-3 h-3 rounded-sm bg-orange-400/30 border border-orange-400/60 inline-block" />
                Non-working ({total} day{total !== 1 ? 's' : ''})
              </span>
            )
          })()}
          <span className="flex items-center gap-1.5">
            <span className="w-4 border-t-2 border-dashed border-rose-400 inline-block" />
            Today
          </span>
        </div>
