...
export const AGENT_SKILLS: Record<string, AgentSkill[]> = {
  ...
  'Interaction Designer': [
    { name: 'Interview Script Design', description: 'Write one interview script per active L1 and L2 node. L1 scripts must ask strategic "why" questions aimed at GMs; L2 scripts must ask operational "how" questions aimed at process managers. Number all questions with n.n.n IDs — never produce an unnumbered question.', icon: FileEdit },
    { name: 'Maturity Questionnaire Design', description: 'For each node, write a five-point maturity questionnaire where level 1 is ad hoc and level 5 is optimised and continuously improving. Align every level descriptor to the configured framework — no descriptor may be written without a traceable standard clause.', icon: ClipboardList },
    { name: 'Coherent Instrument Design', description: 'Design scripts and questionnaires as a paired set for each node. Before finalising, confirm the interview questions and maturity descriptors cover the same capability dimensions — never submit a set where a topic appears in one instrument but not the other.', icon: Target },
    { name: 'Standards Grounding', description: 'Before writing any instrument content, retrieve the configured framework standards from the project setup. Every question and maturity descriptor must be traceable to a specific standard clause or principle — reject content that cannot be traced.', icon: Ruler },
    { name: 'Template Auto-Assignment', description: 'On completing the instrument set, publish each script and questionnaire as a named template and assign it to its value chain node by n.n.n ID. Confirm the assignment in the output before ending the run — unassigned templates are incomplete.', icon: Tags },
    ...
    { name: 'State Management', description: 'Write both interview_scripts.json and questionnaire_scripts.json to the project outputs directory before ending the run. A run that writes only one file is incomplete — both files are required for downstream agents.', icon: Database },
    { name: 'Human Review Gate', description: 'At the end of every work phase, pause and request human review. Write a clear summary of what was produced and what the reviewer needs to validate. Do not allow downstream crews to proceed until review is confirmed.', icon: UserCheck },
  ],
  'Stakeholder Manager': [
