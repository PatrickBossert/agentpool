# SP5a — PAM Pipeline Integration Test Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add a single end-to-end integration test that calls `run_pam_crew()` directly, verifying that all five specialist crews run sequentially, the DB lifecycle is correct, and each crew produced its key output file.

**Architecture:** One new test file in `tests/integration/`. Function-scoped fixtures create an isolated project directory, SQLite DB, and ChromaDB collection for the test run. Nine LLM factory patches via `ExitStack` force all crews to Haiku. `N8N_WEBHOOK_URL=""` suppresses Slack notifications.

**Tech Stack:** pytest, unittest.mock.patch / ExitStack, asyncio.run(), sqlite3, aiosqlite (via `get_connection`), chromadb, yaml

---

### Task 1: Write and run the PAM pipeline integration test

**Files:**
- Create: `tests/integration/test_pam_orchestration.py`

---

- [ ] **Step 1: Write the test file**

Create `tests/integration/test_pam_orchestration.py` with the exact content below.

**Why 9 patches via ExitStack?**  
Every crew factory uses `from agents.llm import get_crew_llm` — a local binding. Patching `agents.llm.get_crew_llm` has no effect because each module already holds a direct reference. You must patch the name at each module that bound it. `ExitStack` lets you enter all 9 context managers cleanly without deep nesting.

**Why `asyncio.run()` twice?**  
The test is a sync function (matching all 6 existing integration tests). The first `asyncio.run(_insert_run())` creates the `orchestration_run` record. The second `asyncio.run(run_pam_crew(...))` executes the pipeline. Both complete before pytest evaluates assertions.

**Why SQLite tables created manually in the fixture?**  
The existing integration conftest uses synchronous `sqlite3` for setup. Keeping that pattern avoids importing `aiosqlite` into fixture code. The `orchestration_runs` table must be included here; it is also created by `get_connection()→init_db()` on first async use, but pre-creating it ensures the project row FK is satisfied when `_insert_run()` runs.
...
- [ ] **Step 2: Run the test to verify it is collected correctly (dry-run)**

```bash
pytest tests/integration/test_pam_orchestration.py --collect-only -q
```

Expected output (no errors, one test collected):
```
tests/integration/test_pam_orchestration.py::test_pam_pipeline_end_to_end
1 test collected in X.XXs
```

If you see `ImportError` or `ModuleNotFoundError` at collection time, fix the import before proceeding.
...
Expected terminal output (after 15–40 minutes):
```
PASSED tests/integration/test_pam_orchestration.py::test_pam_pipeline_end_to_end
1 passed in Xm Xs
```

If the test fails:
- `AssertionError: orchestration_runs.status='failed'` → check pytest output / crew logs for which sub-crew raised. `run_pam_crew` catches all exceptions and sets status to `"failed"`.
- `AssertionError: discovery crew_run not completed` → RunCrewTool swallowed an error; check CrewAI verbose output to see which agent failed.
- `FileNotFoundError` or missing output file → the relevant crew completed its DB record but its file-writing tool errored; look for the tool's warning in stdout.

- [ ] **Step 4: Verify unit tests are unaffected**
...
(Count may vary slightly if other unit tests were added, but there must be zero failures.)

- [ ] **Step 5: Commit**

```bash
git add tests/integration/test_pam_orchestration.py
git commit -m "test(integration): add SP5a PAM pipeline end-to-end test"
```
