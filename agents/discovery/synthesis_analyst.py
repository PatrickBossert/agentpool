...
def create_synthesis_analyst_task(
    ...
    return Task(
        description=(
            ...
            "   An anchor may be ANY registry node - the L0 organisation, an L0 role node "
            "(0.A audit, 0.S corporate services), an L1 entity, an L1 role node (1.C its "
            "customer, 1.F its frontline), an L2 stage or an L3 activity. Anchor each theme "
            "at the level where the insight actually lives:\n"
            ...
            "value propositions built from your themes toward L3 efficiency, because that "
            "becomes the only altitude the evidence is ever expressed at. A governance theme "
            "hung on an arbitrary L3 child is a governance theme nobody can act on at "
            "governance level.\n"
            ...
            "   A requirement anchors the same way a theme does - at the level it must be "
            "acted on, which is usually the level of the themes it derives from. Some "
            "strategic requirements are smaller, tactical items; say so by anchoring them "
            "where they bite rather than inflating them to L0.\n"
