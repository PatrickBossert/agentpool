...
def create_interaction_designer_task(
    ...
    return Task(
        description=(
            ...
            "keyed by node_label. L0, L1, L2, C, A, F, and S scripts include framing_block and synthesis_check. "
            ...
            "nodes than anyone will be interviewed about, and a script for every L3 activity "
            "is a programme nobody would run. Write a script for every L0 and every L1 node, "
            "and for every role node (C, A, F, S), that does not already have one - step 4 "
            "governs this too: a node with an existing script is done, not due for a rewrite. "
            "Below that, select: an L2 stage warrants "
            "a script when it carries a value lever, holds a maturity question the L1 script "
            "cannot answer, or is where one party hands off to another; not every L3 "
            "warrants one, and an L3 does only when that specific activity is itself "
            "contested or is where a lever actually bites. Say in each script's "
            "research_brief why that node was chosen - a selection nobody can see is one "
            "nobody can challenge, and it is what makes the set differ from run to run. "
            "Work through the nodes in registry order and stop when every node you have "
            "chosen has a script.\n"
        ...
        expected_output=(
            ...
            "script per L0 and L1 node, per C (customer), A (audit), F (frontline) and "
            "S (corporate services) role node, and per selected L2 and L3 node; "
            ...
            "one entry per script written, so a registered script id is never filed against a "
